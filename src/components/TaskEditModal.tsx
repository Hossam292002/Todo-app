  ...
  const modal = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <p className="mb-4 text-sm text-slate-900 dark:text-slate-200">
            ...
            <button
              ...
              className={`w-full rounded-lg px-3 py-2 text-left text-sm font-medium transition-colors ${
                c.id === task.category_id
                  ? 'cursor-default bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-300'
                  : 'bg-slate-100 text-slate-800 hover:bg-emerald-100 hover:text-emerald-800 dark:bg-slate-700 dark:text-slate-100 dark:hover:bg-emerald-900/30 dark:hover:text-emerald-300'
              }`}
            >
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button
            onClick={onClose}
            className="flex-1 rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50 dark:border-slate-500 dark:text-slate-100 dark:hover:bg-slate-700"
          >
            Cancel
          </button>
        </div>
